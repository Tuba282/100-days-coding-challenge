...
function my_bad(...bad: string[]) {
  bad.forEach((badness) => {
    console.log(
      `Ullo hon main mujy ${badness}  bhi nahi ata. \n\n English is not comes to me.\n`
    );
  });
}
...
function num(width: number, height: number): number {
  return width * height;
}
let Store = (width: number, height: number): number => width * height;

console.log(num(5, 7));

// lets try another

function security(x:number,y:number) :number{
    return x+y;
}
