...
console.log(
  `\n\tI used to make projrcts like ${
    (projects[0], projects[1], projects[2])
  } with the help of ${(use[0], use[1], use[2])} and now i am studying ${
    known_languages[2]
  }\n\n`
);
...
function getting_something(key: string, value: string) {
  let something = {};
  // @ts-ignore
...
function smart_mobile(key: string, value: string) {
  let now_phone = {};
  //@ts-ignore
  now_phone[key] = value;
  return now_phone;
